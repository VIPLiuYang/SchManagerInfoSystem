<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserFuncSoureInfo.aspx.cs" Inherits="SchWebMaster.Web.UserFunc.UserFuncSoureInfo" %>
...
<body ontouchstart>
    ...
    <%--    <script  type="text/javascript">
        ...
        client.open("GET", "http://139.129.252.49:8181/res/userapi")
    ...
    <script type="text/javascript">
        ...
        //    },
        //    headers: { 'Cookie': 'JSID=c6cc7691-8925-4b20-aec0-78c871498c05; token=4470986918667701122'},
        //    url: "http://139.129.252.49:8181/res/userapi", 
        ...
        function go() {
            ...
            sessionStorage.setItem("userfuncsoureinfo", $("#main-container").html()); //存储列表数据
            //缓存查询条件
            sessionStorage.setItem("userfuncsoureinfo_pageindex", PageIndex);
            sessionStorage.setItem("userfuncsoureinfo_txtname", txtname);
            sessionStorage.setItem("userfuncsoureinfo_dptid", dptid);
            sessionStorage.setItem("userfuncsoureinfo_ustat", ustat);
            sessionStorage.setItem("userfuncsoureinfo_uschid", uschid);
            return false;
        };

        //页面缓存,页面是返回的
        var l = sessionStorage.getItem("userfuncsoureinfo");
        if (l != null) {
            $("#main-container").html(l);            //删除缓存
            sessionStorage.removeItem("userfuncsoureinfo");
            //取回缓存中的查询条件
            PageIndex = sessionStorage.getItem("userfuncsoureinfo_pageindex");
            txtname = sessionStorage.getItem("userfuncsoureinfo_txtname");
            dptid = sessionStorage.getItem("userfuncsoureinfo_dptid");
            ustat = sessionStorage.getItem("userfuncsoureinfo_ustat");
            uschid = sessionStorage.getItem("userfuncsoureinfo_uschid");
            sessionStorage.removeItem("userfuncsoureinfo_pageindex");
            sessionStorage.removeItem("userfuncsoureinfo_txtname");
            sessionStorage.removeItem("userfuncsoureinfo_dptid");
            sessionStorage.removeItem("userfuncsoureinfoustat");
            sessionStorage.removeItem("userfuncsoureinfo_uschid");
        }
        else {
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "UserFuncSoureInfo.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                ...
                $.each(eval("(" + data + ")").list, function (index, item) { //遍历返回的json
                    ...
                    if (isedit == "True") {
                        ...
                        if (item.UserName == "" && item.UserName.length <= 0) {
                            ...
                            str += '<a class="green" href="javascript:updateuser(' + item.UserId + ')" class="tooltip-success" data-rel="tooltip" onclick="go();" title="编辑">';
                            str += '<i class="icon-pencil bigger-130"></i>';
                            str += '</a>';
                        }
                        str += '</li>';
        ...
        function DelRun(schid, userid) {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "UserFuncSoureInfo.aspx/udel",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
        ...
        function CreatUserModal(schid, systype, ostype) {
            if (ostype == "a") {
                ...
                var url = "UserSoure.aspx?dotype=a&schid=" + schid + "&systype=" + systype;
            } else if (ostype == "e") {
                $("#EditCfmCreateUser .modal-title").html("修改人员权限");
                var url = "UserSoure.aspx?dotype=e&uid=" + schid + "&systype=" + systype;
            }
