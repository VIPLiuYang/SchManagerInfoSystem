<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="StudentList.aspx.cs" Inherits="SchWebMaster.Web.Student.StudentList" %>
                ...
                <div class="page-content">
                    <div style="background-color: rgb(239, 243, 248); padding: 6px 4px;">
                        ...
                        <div style="float: right; margin-right: 50px;">
                            学生姓名:&nbsp;<input type="text" id="Stuname" style="width: 150px; height: 30px;" placeholder="请输入学生姓名" />
...
<script type="text/javascript">
    ...
    function dodata(data) {
        if (eval("(" + data + ")").list != null) {
            ...
            text += ' <thead>';
            text += ' <tr>';
            text += ' <th class="hidden-480 center">';
            text += '<label>';
            ...
            text += ' <th>年级</th>';
            text += ' <th>班级</th>';
            text += ' <th class="hidden-480">系统编号</th>';
            text += ' <th>学(考号)</th>';
            text += '<th class="hidden-480">学生姓名</th>';
            text += '<th class="hidden-480">学生性别</th>';
            ...
            $.each(eval("(" + data + ")").list, function (index, item) { //遍历返回的json
                ...
                */
                ...
                text += '<tr>';
                text += '   <td class="hidden-480 center">';
                text += '       <label>';
                text += '           <input type="checkbox" class="ace"  />';
                text += '           <span class="lbl"></span>';
                text += '       </label>';
                ...
                text += '   <td class="hidden-480">' + Stubh + '</td>';
                if (item.TestNo.length > 0) {
                    text += '   <td>' + item.TestNo.substring(0, 1) + '***</td>';
                }
                else {
                    text += '   <td>' + item.TestNo + '</td>';
                }
                text += '<td class="hidden-480" title="' + item.StuName + '">' + (item.StuName == null ? '' : (item.StuName.length > 10 ? item.StuName.substr(0, 10) + '...' : item.StuName)) + '</td>';
                text += '   <td class="hidden-480">' + sex + '</td>';
                text += '   <td class="hidden-480">' + item.CardNo + '</td>';
                if (item.TelNo.length > 0) {
                    text += '   <td class="hidden-480">' + item.TelNo.substr(0, 1) + '***</td>';
                }
                else {
                    text += '   <td class="hidden-480">' + item.TelNo + '</td>';
                }
                ...
                if (item.islook == '1') {
                    text += '	        <a class="green" href="StuShow.aspx?id=' + item.StuId + '&dotype=c&gradecode=' + gradecode + '&classid=' + classid + '&Stubh=' + Stubh + '" onclick="go();">';
                ...
                if (item.islook == '1') {
                    ...
                    text += '<li>';
                    text += '<a  href="StuShow.aspx?id=' + item.StuId + '&dotype=c&gradecode=' + gradecode + '&classid=' + classid + '&Stubh=' + Stubh + '" class="tooltip-info" data-rel="tooltip" title="查看">';
                    text += '<span class="blue">';
                    text += '<i class="icon-zoom-in bigger-120"></i>';
                    text += '</span>';
                    text += '</a>';
                ...
                if (item.isedit == '1') {
                    text += '<li>';
                    text += '<a  href="StuE.aspx?id=' + item.StuId + '&dotype=e&gradecode=' + gradecode + '&classid=' + classid + '&Stubh=' + Stubh + '" onclick="go();" class="tooltip-success" data-rel="tooltip" title="编辑">';
    ...
    $('#bj').change(function () {
        ...
        var selv = $('#bj').val();
        var addstr = '<div class="input-group pull-right">' +
                         ' <a class="btn btn-link" href="StuE.aspx?dotype=a&gradecode=' + selvnj + '&classid=' + selv + '\" onclick="go();">' +
                        ' <i class="icon-plus align-top bigger-125"></i>' +
                             ' 新建学生和家长' +
                         '</a></div>';
        $('#addbtns').html(addstr);
