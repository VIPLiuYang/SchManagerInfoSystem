<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchStructureSet.aspx.cs" Inherits="SchWebMaster.Web.SchStructureSet.SchStructureSet" %>
...
<head>
    ...
    <style type="text/css">
        ...
        .SchClassResult .default {
            margin: 3px 5px;
        }

        .SchClassResult .dropdown {
            margin: 3px 5px;
        }
            ...
            <div class="main-content" style="margin-left: 0px">
                ...
                <div class="breadcrumbs breadcrumb_box " id="breadcrumbs">
                    <ul class="breadcrumb breadcrumb_border ziti">
                        <li>目前位置：</li>
                        <li class="active icon">学校架构设置</li>
                    </ul>
                </div>
                <!--//面包屑导航-->
                <!--面包屑导航右侧的返回按钮-->
                ...
                <div class="page-content">
                    <div class="tabbable">
                        <ul class="nav nav-tabs" id="myTabsub">
                            <li class="active">
                                <a data-toggle="tab" href="#subset">开设科目
                                </a>
                            </li>
                            <li class="">
                                <a data-toggle="tab" href="#classset">班级设置
                                </a>
                            </li>
                            <li class="">
                                <a data-toggle="tab" href="#dptset">部门设置
                                </a>
                            </li>
                        </ul>

                        <div class="tab-content">
                            <div id="subset" class="tab-pane active">
                                <div class="no-padding-left" id="SchOpenSub">
                                    <%=schsubname %>
                                </div>
                            </div>
                            <div id="classset" class="tab-pane">
                                <div class="no-padding-left" id="SchClassSetting">
                    ...
                    <!--基础信息-->
                    <!--<div class="row">
                        <div class="col-sm-1 no-padding text-center subleadertitle">基础信息：</div>
                        <div class="col-sm-11 no-padding-left" id="SchBaxicInfo">
                            学校代码：<span id="schidstr"></span>&nbsp;&nbsp;学校名称：<span id="schnamestr"></span>
                        </div>
                    </div>
                    <div class="space-10"></div>-->
                    <!--开设科目-->
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        //$("#schidstr").html(PreFixInterge(schid,8));
        //$("#schnamestr").html(schnamestr);
    })
    //班级设置--Start
    //判断数组是否是从1开始的连续数字
    ...
    function doclassdom(data){
        if (schgradeinfo != null && schgradeinfo != "") {
            ...
            $.each(schgradeinfo, function (index, item) { //遍历返回的json
                schgradedom +="<tr>";
                schgradedom +="     <td class=\"text-center\" style=\"vertical-align:middle\">"+item.GradeName+"</td>";
                schgradedom +="     <td>";
                schgradedom +="         <div id=\"SchClassResult"+item.GradeId+"\" class=\"SchClassResult fl\" style=\"vertical-align:middle\">";
                //schgradedom +="             <span>";
                var isclassval=0;
                $.each(data,function(indexs,items){
                    if(item.GradeId==items.GradeId){
                        schgradedom +="<div class=\"item fl\" val=\"" + items.ClassId + "\" ><input type=\"hidden\" name=\"valname\" gradeid=\""+item.GradeId+"\" gradecode=\""+item.GradeCode+"\" value=\""+items.ClassName+"\" />" + items.ClassName ;
                ...
                if(isclassval==0){
                    schgradedom +=" <span class=\"default fl\">请创建新班级</span>";
                }
                //schgradedom +="             </span>";
                if (isadd == "True") {
                    schgradedom +="     <div class=\"dropdown fl\" style=\"vertical-align:middle\">";
                    schgradedom +="         <p gradeyear=\""+item.GradeYear+"\" gradeid=\""+item.GradeId+"\" gradecode=\""+item.GradeCode+"\" gradename=\""+item.GradeName+"\" style=\"vertical-align:middle\">添加班级<span>&or;</span></p>";
                    schgradedom +="     </div>";
                }
                schgradedom +="         </div>";
    ...
    $(document).on("click",".dropdown p",function(){
        ...
        var chkboxdom = "";
        var substringstr = $(this).attr("gradeyear");//undefined
        var schgradeidstr = $(this).attr("gradeid");
        var schgradecodestr = $(this).attr("gradecode");
        ...
        var gradeyearsubstr = substringstr.substring(2);
        chkboxdom += "<div class=\"row\">";
        chkboxdom += "<input type=\"hidden\" name=\"schgradeidstr\" value=\""+schgradeidstr+"\" />";
        chkboxdom += "<input type=\"hidden\" name=\"schgradecodestr\" value=\""+schgradecodestr+"\" />";
        ///
        for(var i=1;i<=54;i++){
            chkboxdom += "<div class=\"col-sm-2\">";
            if(i<10){
                chkboxdom += "<input class=\"fl\" type=\"checkbox\" name=\"schclassname\" value=\""+gradeyearsubstr+"0"+i+"班\"";
                for(var j=0;j<10;j++){
                    if($(selectitem[j]).val()==gradeyearsubstr+"0"+i+"班"){
                        chkboxdom += " checked=\"checked\" disabled=\"disabled\" ";
                    }
                }
                chkboxdom += "/><div class=\"classtit fl\">"+gradeyearsubstr+"0"+i+"班</div> ";
            }else{
                chkboxdom += "<input class=\"fl\" type=\"checkbox\" name=\"schclassname\" value=\""+gradeyearsubstr+i+"班\"";
                for(var j=0;j<54;j++){
                    if($(selectitem[j]).val()==gradeyearsubstr+i+"班"){
                        chkboxdom += " checked=\"checked\" disabled=\"disabled\" ";
                    }
                }
                chkboxdom += "/><div class=\"classtit fl\">"+gradeyearsubstr+i+"班</div> ";
            }
    ...
    function SchClassSaveData(){
        ...
        var schgradecodestr = $("input:hidden[name=\"schgradecodestr\"]").val();
        ...
        if(classnamestr.toString()!=""){
            ...
            var params = '{"schid":"' + schid + '","gradeid":"' + schgradeidstr + '","gradecode":"' + schgradecodestr + '","classnamestr":"' + classnamestr.toString() + '"}';
            $.ajax({
                type: "POST",  //请求方式
    ...
    function savedpt() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.d == 'success') {                    
                    //更新关联节点角色名称
                    var enode=treedeptObj.getNodeByParam("id", id, null);
                    if(enode!=null)
                    {
                        enode.name=dptname;
                        treedeptObj.updateNode(enode);
                    }
                    alert("修改成功");
