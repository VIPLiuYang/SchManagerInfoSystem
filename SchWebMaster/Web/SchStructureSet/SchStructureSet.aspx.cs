...
namespace SchWebMaster.Web.SchStructureSet
{
    public partial class SchStructureSet : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //DataTable dtschinfo = siBll.GetList("SchName", "SchId='" + Com.Session.schid + "'").Tables[0];
                //if (dtschinfo.Rows.Count > 0)
                //{
                //    schname = dtschinfo.Rows[0]["SchName"].ToString();
                //}
                //获取学校的科目
                ...
                //DataTable dtschsub = SchSubBll.GetSubList("SchSub.Stat=1 and SchId='" + Com.Public.SqlEncStr(schid) + "' Order by SchSub.SubCode").Tables[0];
                ...
                DataTable dtschgrade = SchGradeBll.GetList("GradeId,GradeYear,GradeCode,GradeName", "IsFinish=0 and SchId='" + Com.Public.SqlEncStr(schid) + "' Order by GradeCode ASC").Tables[0];
        ...
        public static int ClassDelete(string schid, string schclassid)
        {
            ...
            bool resultBoolUser = classuserbll.ExistsClassUser(schid, int.Parse(schclassid));
        ...
        public static string getclassdatas(string schid)
        {
            string ret = "";
            SchSystem.BLL.SchClassInfo SchClassBll = new SchSystem.BLL.SchClassInfo();
            DataTable dtschclass = SchClassBll.GetList("ClassId,GradeId,ClassNo,ClassName,GradeCode", "IsFinish=0 and SchId='" + schid + "' order by ClassName asc").Tables[0];
