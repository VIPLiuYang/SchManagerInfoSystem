<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserInfo.aspx.cs" Inherits="SchWebMaster.Web.Users.UserInfo" %>
...
<body ontouchstart>
    ...
    <script type="text/javascript">
        ...
        function dodata(data) {
            ...
            if (eval("(" + data + ")").list != null) {
                ...
                $.each(eval("(" + data + ")").list, function (index, item) { //遍历返回的json
                    ...
                    str += '<tr>';
                    str += '<td>' + i + '</td>';
                    str += '<td class="hidden-480">' + PreFixInterge(item.UserId, 8) + '</td>';
                    str += '<td title="' + item.UserTname + '">' + (item.UserTname == null ? '' : (item.UserTname.length > 10 ? item.UserTname.substr(0, 10) + '...' : item.UserTname)) + '</td>';
                    str += '<td class="hidden-480">' + item.Sexn + '</td>';
                    str += '<td class="hidden-480" title="' + item.Dpts + '">' + (item.Dpts == null ? '' : (item.Dpts.length > 10 ? item.Dpts.substr(0, 10) + '...' : item.Dpts)) + '</td>';

                    str += '<td class="hidden-480" title="' + item.Postion + '">' + (item.Postion == null ? '' : (item.Postion.length > 10 ? item.Postion.substr(0, 10) + '...' : item.Postion)) + '</td>';

                    str += '<td class="hidden-480" title="' + item.Title + '">' + (item.Title == null ? '' : (item.Title.length > 10 ? item.Title.substr(0, 10) + '...' : item.Title)) + '</td>';
                    str += '<td class="hidden-480">' + item.Mobile + '</td>';
                    if (item.UserName != null && item.UserName.length > 0) {
                        str += '<td class="hidden-480">' + item.UserName.substring(0, 1) + '***</td>';
                    }
                    else {
        ...
        $('#aprov').change(function () {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "UserInfo.aspx/getarea",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
        ...
        function CreatUserModal(schid, systype, ostype) {
            if (ostype == "a") {
                ...
                var url = "UserE.aspx?dotype=a&schid=" + schid + "&systype=" + systype;
            } else if (ostype == "e") {
                $("#EditCfmCreateUser .modal-title").html("修改人员");
                var url = "UserE.aspx?dotype=e&uid=" + schid + "&systype=" + systype;
            }
            else if (ostype == "s") {
