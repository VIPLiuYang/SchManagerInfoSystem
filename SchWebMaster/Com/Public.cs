...
namespace SchWebMaster.Com
{
    public class Public
    {
        ...
        public static string GetGradeSelect(string typecode, int classid, ref string id, string gradeid)
        {
            ...
            if (bllclassuser.ExistsIsMs(Com.Public.SqlEncStr(classid.ToString()), Com.Session.usertid, Com.Session.schid, 1) == true)//班主任
            {
                //年级条件
            ...
            //if (bll_stu.ExistsGrade(Com.Public.SqlEncStr(gradeid.ToString()), Com.Session.schid, Com.Session.usertid) == true)//年级领导
            //{
            //    sbGradewhere.Append("GradeId in (select GradeId FROM SchGradeUsers where UserName=" + Com.Session.usertid + ")");
            //}
        ...
        public static string GetDrpArea(string TypeCode, string pAreaCode, ref string sAreaCode, bool addall)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                if (sAreaCode == "" || sAreaCode == "-1")//输出第一个节点
                {
                    sAreaCode = dt.Rows[0]["ID"].ToString();
                }
        ...
        public static string UserLoginDo(string uname, bool iscookies, string schid)
        {
            ...
            //查询登录用户记录数据
            DataTable SchUserInfoDs = userbll.GetList("UserId,UserName,UserTname,SchId,SysType,ImgUrl", "UserName='" + Com.Public.SqlEncStr(uname) + "' and Stat=1 and AccStat=1 and SysType=1").Tables[0];// and SchId=" + schid
