...
namespace SchSystem.DAL
{
	...
	public partial class SchGenInfo
	{
        ...
        /// 得到一个对象实体
        /// </summary>
        public int GetMbId(string TelNo)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 GenId,TelNo from SchGenInfo ");
            strSql.Append(" where TelNo=@TelNo ");
            SqlParameter[] parameters = {
					new SqlParameter("@TelNo", SqlDbType.VarChar,50)			};
            parameters[0].Value = TelNo;
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return int.Parse(ds.Tables[0].Rows[0]["GenId"].ToString());
            }
            else
            {
                return 0;
        ...
        ///// 是否存在手机号
        ///// </summary>
        //public int ExistsTelNo(string strWhere)
        //{
        //    StringBuilder strSql = new StringBuilder();
        //    strSql.Append(" select count(1) as aaa,GenId from	 SchGenInfo "); 
       ...
       // }
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select GenId,LoginName,Pwd,GenName,Sex,ImgUrl,Stat,TelNo,LoginTime,RecTime,RecUser,LastRecTime,LastRecUser ");
            strSql.Append(" FROM SchGenInfo with(nolock) ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
