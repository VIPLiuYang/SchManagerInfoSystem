...
namespace SchSystem.DAL
{
    ...
    public partial class SchUserRole
    {
        ...
        public int DoUserRole(string UserName, string RecUser, string SchId,string RoleIds)
        {
            ...
            strSql.Append("delete from SchUserRole where UserName='" + UserName + "' and SchId=" + SchId);
            strSql.Append(" insert into SchUserRole(RoleID,UserName,RecTime,RecUser,LastRecTime,LastRecUser,SchId)");
            strSql.Append(" select RoleID,'" + UserName + "',getdate(),'" + RecUser + "',getdate(),'" + RecUser + "','" + SchId + "' from SchRole where RoleID in (" + RoleIds + ")");
