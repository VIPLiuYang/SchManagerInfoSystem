<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchXXTBasicerList.aspx.cs" Inherits="SchWebAdmin.Web.SchXXTBasicer.SchXXTBasicerList" %>
...
<body ontouchstart>
    ...
    <script type="text/javascript">
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchXXTBasicerList.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
        ...
        function dodata(data) {
            if (data.list != null) {
                ...
                str += '<th class="hidden-480">数据维护</th>';
                ...
                $.each(data.list, function (index, item) { //遍历返回的json
                    ...
                    str += '<td class="hidden-480" title="' + item.HomeSchPlatIP + '">' + (item.HomeSchPlatIP == null ? '' : (item.HomeSchPlatIP.length > 10 ? item.HomeSchPlatIP.substr(0, 10) + '...' : item.HomeSchPlatIP)) + '</td>';
                    ...
                    str += '<td class="hidden-480" title="' + item.SoureName + '">' + (item.SoureName == null ? '' : (item.SoureName.length > 4 ? item.SoureName.substr(0, 4) + '...' : item.SoureName)) + '</td>';
                    if (item.HomeSchBasicStat == "0") {
                        str += '<td class="hidden-480">客服维护</td>';
                    } else if (item.HomeSchBasicStat == "1") {
                        str += '<td class="hidden-480">学校维护</td>';
                    } else {
        ...
        function DelRun(id) {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchXXTBasicerList.aspx/udel",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
        ...
        function CreatUserModal(schid, systype, ostype) {
            if (ostype == "a") {
                ...
                var url = "SchXXTBasicerEdit.aspx?dotype=e&schid=" + schid + "&systype=" + systype;
            }
            else if (ostype == "s") {
