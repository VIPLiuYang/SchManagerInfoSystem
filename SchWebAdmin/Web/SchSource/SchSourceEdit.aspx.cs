...
{
    public partial class SchSourceEdit : System.Web.UI.Page
    {
        ...
        public static Com.DataPack.DataRsp<string> schsubsave(string arr)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (datas[1] == "1")
                    {
                        schbll.UpdateSoureSysOpen(int.Parse(datas[0]), DateTime.Parse(datas[2]), "支撑操作开启");
                        //自动更新家校互通平台的数据维护人信息
                        schbll.UpdateSchXXTServStat(int.Parse(datas[0]), int.Parse(datas[1]));
                    }
                    else
                    {
                        schbll.UpdateSoureSysClose(int.Parse(datas[0]), "支撑操作关闭");
                    }
                    rsp.code = "success";
                    rsp.msg = "操作成功";
