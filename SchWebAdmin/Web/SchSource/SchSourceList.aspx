<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchSourceList.aspx.cs" Inherits="SchWebAdmin.Web.SchSource.SchSourceList" %>
...
<body ontouchstart>
            ...
            <div class="main-content" style="margin-left: 0px">
                ...
                <div class="page-content content_box ">
                                    <div  style="background-color: rgb(239, 243, 248);text-align: right; padding: 6px 4px;">
                                        ...
                                        学校代码:<input type="text" id="txtschid" placeholder="学校代码">服务状态:
                                    <select id="ustat">
    ...
    <script type="text/javascript">
        ...
        var islook = '<%=islook%>';
        function go() {
            sessionStorage.setItem("schsourelistindex", $("#main-container").html()); //存储列表数据
            //缓存查询条件
            sessionStorage.setItem("schsourelistindex_pageindex", PageIndex);
            sessionStorage.setItem("schsourelistindex_pagesize", PageSize);
            sessionStorage.setItem("schsourelistindex_pagecount", pageCount);
            sessionStorage.setItem("schsourelistindex_txtschid", txtschid);
            sessionStorage.setItem("schsourelistindex_cotycode", cotycode);
            sessionStorage.setItem("schsourelistindex_schname", schname);
            sessionStorage.setItem("schsourelistindex_ustat", ustat);
            sessionStorage.setItem("schsourelistindex_aprovserch", aprovserch);
            sessionStorage.setItem("schsourelistindex_acityserch", acityserch);
            return false;
        };
        //页面缓存,页面是返回的
        var l = sessionStorage.getItem("schsourelistindex");
        if (l != null) {
            $("#main-container").html(l);            //删除缓存
            sessionStorage.removeItem("schsourelistindex");
            //取回缓存中的查询条件
            PageIndex = sessionStorage.getItem("schsourelistindex_pageindex");
            PageSize = sessionStorage.getItem("schsourelistindex_pagesize");
            pageCount = sessionStorage.getItem("schsourelistindex_pagecount");
            txtschid = sessionStorage.getItem("schsourelistindex_txtschid");
            cotycode = sessionStorage.getItem("schsourelistindex_cotycode");
            schname = sessionStorage.getItem("schsourelistindex_schname");
            ustat = sessionStorage.getItem("schsourelistindex_ustat");
            aprovserch = sessionStorage.getItem("schsourelistindex_aprovserch");
            acityserch = sessionStorage.getItem("schsourelistindex_acityserch");
            sessionStorage.removeItem("schsourelistindex_pageindex");
            sessionStorage.removeItem("schsourelistindex_pagesize");
            sessionStorage.removeItem("schsourelistindex_pagecount");
            sessionStorage.removeItem("schsourelistindex_txtschid");
            sessionStorage.removeItem("schsourelistindex_cotycode");
            sessionStorage.removeItem("schsourelistindex_schname");
            sessionStorage.removeItem("schsourelistindex_ustat");
            sessionStorage.removeItem("schsourelistindex_aprovserch");
            sessionStorage.removeItem("schsourelistindex_acityserch");
        }
        else {
            schid = '<%=schid%>';//第一次赋给ID
            cotycode = '<%=cotycode%>';
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchSourceList.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
        ...
        function dodata(data) {
            if (data.list != null) {
                ...
                str += '<th class="hidden-480">资源服务备注</th>';
                ...
                $.each(data.list, function (index, item) { //遍历返回的json
                    ...
                    } else if (item.SourceSerStat == "0") {
                        str += '<td class="hidden-480">停用</td>';
                    } else {
                        str += '<td class="hidden-480"></td>';
                    }

                    if (item.SourceSerStatNote != null) {
                        str += '<td class="hidden-480">' + item.SourceSerStatNote + '</td>';
                    } else {
                        str += '<td class="hidden-480"></td>';
                    }
                    ...
                    if (item.SoureSerEnableTime != null && item.SourceSerStat == "1") {
                        str += '<td class="hidden-480">' + item.SoureSerEnableTime.substring(0, 10) + '</td>';
                    } else {
                        str += '<td class="hidden-480"></td>';
                    }

                    if (item.SourceSerEndTime != null && item.SourceSerStat == "1") {
                        str += '<td class="hidden-480">' + item.SourceSerEndTime.substring(0, 10) + '</td>';
                    } else {
                        str += '<td class="hidden-480"></td>';
                    }
                    str += '<td>';
        ...
        function DelRun(id) {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchSourceList.aspx/udel",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
        ...
        function CreatUserModal(schid, systype, ostype) {
            if (ostype == "a") {
                ...
                $("#EditCfmCreateUser .modal-title").html("启用停用资源服务");
                var url = "SchSourceEdit.aspx?dotype=e&schid=" + schid + "&systype=" + systype;
            }
            else if (ostype == "s") {
