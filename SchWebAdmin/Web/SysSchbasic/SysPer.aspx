<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysPer.aspx.cs" Inherits="SchWebAdmin.Web.SysSchbasic.SysPer" %>
...
<body>
    ...
    <div class="modal fade" id="EditCfmCreate">
        <div class="modal-dialog" style="width: 50%; height: 100%;">
            <div class="modal-content message_align" style="width: 100%; height: 100%; margin: 0 auto">
    ...
    <script type="text/javascript">
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SysPer.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
        ...
        function update(id, dotype) {
            ...
            CreatModal(id, dotype);
        }
        // 打开新建修改查看人员模态框，函数名：showGradeEditeModal
        var rel_id = ""; var rel_gradecode = "";
        function CreatModal(id, systype) {
            $("#EditCfmCreate .modal-title").html("添加/修改学段");
            var url = "SysPerA.aspx?id=" + id + "&systype=" + systype;
        ...
        function dodata(data) {
            if (data.list != null) {
                ...
                $.each(data.list, function (index, item) { //遍历返回的json 
                    ...
                    str += '<td class="center">' + item.PerCode + '</td>';
                    str += '<td>' + item.PerName +'('+ item.PerYear +'年制)'+ '</td>';
                    if (item.Stat == "1") {
                    ...
                    str += '<td>';
                    str += '<a href ="javascript:update(' + item.AutoId + ',1)" >';
                    str += "<span class='blue'><i class='icon-edit bigger-120'></i></span></a> &nbsp;";
                    //str += "<a href='javascript:del(" + item.AutoId + "," + item.PerCode + ")' class='tooltip-error' data-rel='tooltip' title='删除'>";
