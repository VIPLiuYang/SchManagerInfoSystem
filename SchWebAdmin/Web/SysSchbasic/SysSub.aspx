<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysSub.aspx.cs" Inherits="SchWebAdmin.Web.SysSchbasic.SysSub" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        function CreatModal(autoid, systype) {
            $("#EditCfmCreate .modal-title").html("添加/修改科目");
            var url = "SysSubDialog.aspx?autoid=" + autoid + "&systype=" + systype;
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SysSub.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
            ...
            if (data.list != null) {
                ...
                $.each(data.list, function (index, item) { //遍历返回的json 
                    ...
                    if (item.RecTime != null) {
                        item.RecTime = item.RecTime.substring(0, 9);
                    }
                    str += '<tr>';
                    str += '<td class="center">' + i + '</td>';
                    str += '<td class="center">' + item.SubCode + '</td>';
