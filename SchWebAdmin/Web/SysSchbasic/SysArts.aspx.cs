...
{
    public partial class SysArts : System.Web.UI.Page
    {
        ...
        public static Com.DataPack.DataRsp<string> page(string PageIndex, string PageSize)
        {
            ...
            else
            {
                try
                {
                    SchSystem.BLL.SysArts sysarts = new SchSystem.BLL.SysArts();
                    ...
                    DataTable dt = sysarts.GetListCols("AutoId,ArtsName,ArtsCode,Stat", strwhere, "ArtsCode", "ASC", pages.PageIndex, pages.PageSize, ref rowc, ref pc).Tables[0];
                    pages.PageCount = pc;
                    pages.RowCount = rowc;
                    if (dt.Rows.Count > 0)
                    {
