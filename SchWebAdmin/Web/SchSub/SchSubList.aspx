<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchSubList.aspx.cs" Inherits="SchWebAdmin.Web.SchSub.SchSubList" %>
...
<body ontouchstart>
    ...
    <script type="text/javascript">
        ...
        function go() {
            ...
            sessionStorage.setItem("schsubindex", $("#main-container").html()); //存储列表数据
            //缓存查询条件
            sessionStorage.setItem("schsubindex_pageindex", PageIndex);
            sessionStorage.setItem("schsubindex_txtname", txtname);
            sessionStorage.setItem("schsubindex_cotycode", cotycode);
            sessionStorage.setItem("schsubindex_ustat", ustat);
            sessionStorage.setItem("schsubindex_uschid", schid);
            return false;
        };

        //页面缓存,页面是返回的
        var l = sessionStorage.getItem("schsubindex");
        if (l != null) {
            $("#main-container").html(l);            //删除缓存
            sessionStorage.removeItem("schsubindex");
            //取回缓存中的查询条件
            PageIndex = sessionStorage.getItem("schsubindex_pageindex");
            txtname = sessionStorage.getItem("schsubindex_txtname");
            cotycode = sessionStorage.getItem("schsubindex_cotycode");
            ustat = sessionStorage.getItem("schsubindex_ustat");
            schid = sessionStorage.getItem("schsubindex_uschid");
            sessionStorage.removeItem("schsubindex_pageindex");
            sessionStorage.removeItem("schsubindex_txtname");
            sessionStorage.removeItem("schsubindex_cotycode");
            sessionStorage.removeItem("schsubindex_ustat");
            sessionStorage.removeItem("schsubindex_uschid");
        }
        else {
            schid = '<%=schid%>';//第一次赋给ID
            cotycode = '<%=cotycode%>';
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchSubList.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
        ...
        function dodata(data) {
            if (data.list != null) {
                ...
                //str += '<th class="hidden-480">平台图标</th>';
                ...
                str += '<th class="hidden-480">子系统备注</th>';
                ...
                $.each(data.list, function (index, item) { //遍历返回的json
                    ...
                    str += '<td title="' + item.SchoolSections + '">' + (item.SchoolSections == null ? '' : (item.SchoolSections.length > 4 ? item.SchoolSections.substr(0, 4) + '...' : item.SchoolSections)) + '</td>';
                    ...
                    if (item.SonSysStatNote != null) {
                        str += '<td class="hidden-480">' + item.SonSysStatNote + '</td>';
                    } else {
                        str += '<td class="hidden-480"></td>';
                    }
        ...
        function DelRun(id) {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchSubList.aspx/udel",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
        ...
        function CreatUserModal(schid, systype, ostype) {
            if (ostype == "a") {
                $("#EditCfmCreateUser .modal-title").html("启用停用子系统");
                var url = "SchSubEdit.aspx?dotype=a&schid=" + schid + "&systype=" + systype;
            } else if (ostype == "e") {
                $("#EditCfmCreateUser .modal-title").html("启用停用子系统");
                var url = "SchSubEdit.aspx?dotype=e&schid=" + schid + "&systype=" + systype;
            }
            else if (ostype == "s") {
