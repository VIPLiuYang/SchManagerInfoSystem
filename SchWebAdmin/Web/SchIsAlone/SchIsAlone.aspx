<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchIsAlone.aspx.cs" Inherits="SchWebAdmin.Web.SchIsAlone.SchIsAlone" %>
...
<body ontouchstart>
    ...
    <script type="text/javascript">
        ...
        function getdata() {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchIsAlone.aspx/page",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
        ...
        function dodata(data) {
            if (data.list != null) {
                ...
                str += '<th class="hidden-480">单独部署</th>';
                str += '<th class="hidden-480">操作人</th>';
                str += '<th class="hidden-480">操作时间</th>';
                str += '</tr>';
                str += '</thead>';
                str += '<tbody>';
                var i = 1;
                ...
                $.each(data.list, function (index, item) { //遍历返回的json
                    ...
                    if (item.IsAlone == "1") {
                        str += '<td class="SchIsAlone" schid="' + item.SchId + '" style="color:green;cursor:pointer;">&radic;</td>';
                    } else {
                        str += '<td class="SchIsAlone" schid="' + item.SchId + '" style="color:red;cursor:pointer;">&times;</td>';
                    }
                    if (item.AloneUser) {
                        str += '<td>' + item.AloneUser + '</td>';
                    } else {
                        str += '<td></td>';
                    }
                    if (item.AloneTime) {
                        str += '<td>' + item.AloneTime.substring(0, 10).toString() + '</td>';
                    } else {
                        str += '<td></td>';
                    }
                    i++;
        ...
        $(document).on("click", ".SchIsAlone", function () {
            var isalone = ""; var id = "";
            if ($(this).html() == "×") {
                isalone = "&times;";//×
            } else {
            ...
            id = $(this).attr("schid");
            var params = '{"id":"' + id + '","isalone":"' + isalone + '"}';
            $.ajax({
                type: "POST",  //请求方式
                url: "SchIsAlone.aspx/isalone",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
