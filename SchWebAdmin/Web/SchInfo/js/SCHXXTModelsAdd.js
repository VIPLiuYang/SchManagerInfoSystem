...
$(function () {
    if (treeNodeHomSch) {
        for (var x in treeNodeHomSch) {
            if (treeNodeHomSch[x].checked == 'true') {
                $('#tags-homeSching').tagsinput('add', { id: treeNodeHomSch[x].id, text: treeNodeHomSch[x].name });
            }

        }
    }
    treehomeschObj = $.fn.zTree.init($("#treehomesch"), settinghomsch, treeNodeHomSch);
