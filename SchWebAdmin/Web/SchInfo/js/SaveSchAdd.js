...
function saveuser() {
    ...
    if (!checkTxt("#ManagerAcount", '(^$)|^([a-zA-Z0-9]{6,18})$')) {
    ...
    if (manageracount == "") { alert("请输入管理员账号"); return false; }
    ...
    //GradeCode|GradeYear|GradeName|IsFinish
    for (var i = 0; i < gradechklen; i++) {
        ...
        if ($(gradechkval[i]).is(":checked")) {
            gradechk += gradecodestr + "|" + startyearstr + "|" + gradenamestr + "|0,";
        } else {
            gradechk += gradecodestr + "|" + startyearstr + "|" + gradenamestr + "|2,";
        }
    }
    ...
    if (resourcearr) {
        for (var i = 0; i < resourcearr.length; i++) {
            resourceModules += resourcearr[i] + ",0|";
        }
    ...
    $.ajax({
        type: "POST",  //请求方式
        url: "SchAdd.aspx/schsave",  //请求路径：页面/方法名字
