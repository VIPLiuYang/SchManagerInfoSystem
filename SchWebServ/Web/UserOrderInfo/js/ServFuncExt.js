...
function ServFuncExtTreeOnCheck(event, treeId, treeNode) {
    ...
    var sno = $("#" + treeId).attr("sno");
    var selOplen = $("#" + tagid + " option").size();
    
    if (treeNode.checked) {
        if (selOplen < sno) {
            $("#" + tagid).tagsinput('add', { id: treeNode.id, text: treeNode.name });
        } else {
            var treeObjss = $.fn.zTree.getZTreeObj(treeId);
            var nodess = treeObjss.getNodeByParam("id", treeNode.id, null);
            treeObjss.checkNode(nodess, false, false);
            alert("你选择的超出了个数，请到业务功能管理中设置");
        }
    } else {
