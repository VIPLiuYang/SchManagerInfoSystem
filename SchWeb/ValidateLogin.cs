...
namespace SchWeb
{
    public class ValidateLogin : IHttpModule
    {
        ...
        void context_PreRequestHandlerExecute(object sender, EventArgs e)
        {
            ...
            if (path.IndexOf("viewimg.aspx") > 0 || path.IndexOf("default.aspx") > 0 || path.IndexOf("login") > 0 || path.IndexOf("index.aspx") > 0 || path.IndexOf("userapi.aspx") > 0 || path.IndexOf("userfuncsoureinfo.aspx") > 0)
            {
                return;
            }
            if (tag != -1)
