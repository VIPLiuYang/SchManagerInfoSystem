//邮箱验证
function SubmitEmailCk(idval) {
    var reg = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/;
    if (!reg.test($("#" + idval).val())) {
        alert("请输入正确邮箱地址")
        return false;
    }
}
//手机号码验证
function SubmitPhoneCk(idval) {
    var reg = /^(?:13\d|15[89])-?\d{5}(\d{3}|\*{3})$/;//13,158,159开头
    if (!reg.test($("#" + idval).val())) {
        alert("请输入正确手机号码")
        return false;
    }
...
//手机号码和固定电话数据验证
function SubmitTelPhoneCk(idval) {
    var reg = /(^[0-9]{3,4}\-[0-9]{7,8}$)|(^[0-9]{7,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13|15[0-9]{9}$)/;
    if (!reg.test($("#" + idval).val())) {
        alert("请输入正确手机号码（格式：15153585510）或固定电话（格式：0531-12345678")
        return false;
    }
}
//检查文本框的值：必填,字母和数字的长度6-18位组合
function checkTxtstr(o, reg) {
    var regu = reg;
...
//检查文本框的值：必填,字母和数字的长度6-18位组合
function checkTxts(o, reg, targetid) {
    var regu = reg;
    var re = new RegExp(regu);
    if (re.test($(o).val())) {
        if (targetid != "") { document.getElementById(targetid).innerHTML = "<span style=\"color:green;\">*填写正确</span>"; }
        return true;
    } else {
        if (targetid != "") { document.getElementById(targetid).innerHTML = "<span style=\"color:red;\">*填写不正确</span>"; }
        $(o).tooltip('show');
        $(o).focus();
        return false;
    }
...
function ValNum(obj) {
    if (obj.value.replace(/[\d+]/ig, "").length > 0) {
        alert('只能输入数字或字母'); obj.focus(); obj.value = "";
    }
}
...
function isChinese(obj) {
    if (obj.value.replace(/^[\u0391-\uFFE5]+$/, "").length > 0) {
        alert('只能输入中文'); obj.focus(); obj.value = "";
    }
...
function isInteger(obj) {
    if (obj.value.replace(/^[-+]?\d+$/, "").length > 0) {
        alert('只能输入数字'); obj.focus(); obj.value = "";
    }
}
