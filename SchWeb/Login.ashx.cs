                    ...
                    //{
                        ...
                        if (recode(txtcode, context))
                        {
                            if (!string.IsNullOrEmpty(uname))
                            {
                                ...
                                {
                                    result = userbll.Exists("UserName='" + uname + "' and PassWord='" + pwdmd5 + "' and Stat=1 and AccStat=1 and SysType=0 and schid=" + Com.Public.getKey("appschid"));
                                }
                                else//总平台不允许分学校部署的用户登录,则加上schid所在学校限制
                                {
                                    result = userbll.Exists("UserName='" + uname + "' and PassWord='" + pwdmd5 + "' and Stat=1 and AccStat=1 and SysType=0 and schid not in (select schid from SchInfo where IsAlone=1)");
                                }
                                if (result == true)
                                {
