...
dTree.prototype.node = function (node, nodeId) {
    ...
    if (node.pid == -1) {
        ...
        if (node.pid == "0") {
            str += '<div class="dTreeNodeParent" id="dTreeNodeParent'+node.pid+'">' + this.indent(node, nodeId);
        } else {
            str += '<div class="dTreeNodeChild">' + this.indent(node, nodeId);
        }
    }
    ...
    if (node.url) {
        str += '<a id="d' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url + '"';
...
dTree.prototype.checkNode = function (id, pid, _hc, checked) {
    ...
    $(".dTreeNodeParent").click(function () {
        var is_check = $(this).find('input').get(0).checked; //判断是否选中
        $(this).next().find('input').prop("checked", is_check);
    });
    $(".dTreeNodeChild>a>input").click(function () {
        $(this).parent("a").parent("div").parent("div").children('.dTreeNodeParent').find('a>input').prop('checked', 'checked')
    });
