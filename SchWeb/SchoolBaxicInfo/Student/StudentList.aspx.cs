    ...
    public partial class StudentList : System.Web.UI.Page
    {
        ...
        public static string page(string PageIndex, string PageSize, string ClassId, string SchId, string Stuname, string GradeId)
        {
            ...
            else//普通老师账号
            {
                ...
                {
                    isIsMs = bllclassuser.ExistsIsMs(Com.Session.usertid, Com.Session.schid, 1);
                }
                else
                {
                   isIsMs = bllclassuser.ExistsIsMs(Com.Public.SqlEncStr(ClassId), Com.Session.usertid, Com.Session.schid, 1);
                }
                if (isIsMs == true)//班主任
                {
                    if (Com.Session.appeditstat == "1")
                ...
                else
                {
                    ...
                    {
                        bool s = bllclassuser.ExistsIsMs(Com.Session.usertid, Com.Session.schid, 0);
                        if (bllclassuser.ExistsIsMs(Com.Session.usertid, Com.Session.schid, 0) == true)//任课老师
                        {
                            islist = true;
                        }
                    }
