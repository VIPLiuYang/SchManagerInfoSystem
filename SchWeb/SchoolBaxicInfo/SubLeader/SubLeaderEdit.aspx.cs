...
{
    public partial class SubLeaderEdit : System.Web.UI.Page
    {
        //进入此页需要确认的三个关键参数
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    deptusers = Newtonsoft.Json.JsonConvert.SerializeObject(dtdptuser).Replace("\r\r\n","").Replace("&nbsp;","");
                ...
                depts = sb.ToString().Replace("\r\r\n", "").Replace("&nbsp;", "").Replace("&nbsp;", "");
                deptss = Newtonsoft.Json.JsonConvert.SerializeObject(dtclassuser).Replace("&nbsp;", "").Replace("&nbsp;", "");
                ...
                if (!string.IsNullOrEmpty(gradecode))
                {
                    ...
                    gradeboss = usergradebll.GetNames("GradeId in ( select GradeId from SchGradeInfo where GradeId=" + Com.Public.SqlEncStr(gradecode) + " and schid=" + schid + ")");
                }
        ...
        public static void BindSon(StringBuilder sb, string pid, DataTable dt, string blank, DataTable dtuser)
        {
            ...
            foreach (DataRow dr in drs)
            {
                ...
                if (drusers != null && drusers.Length > 0)
                {
                    foreach (DataRow druser in drusers)
                    {
                        sb.Append("<option style=\"color:blue\"  value=\"u_" + druser["UserId"].ToString() + "\">" + blank2 + "&nbsp;&nbsp;&nbsp;&nbsp;" + druser["UserTname"].ToString() + "</option>");
                    }
                }
