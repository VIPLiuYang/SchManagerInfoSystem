<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchClassEdit.aspx.cs" Inherits="SchWeb.SchoolBaxicInfo.GradeClassSet.SchClassEdit" %>
                            ...
                            <form class="form-horizontal" role="form">
                                ...
                                <div class="form-group form-inline">
                                    <div class="col-sm-12">
                                        ...
                                        <div class="col-sm-3 no-padding-right">
                                            <input type="text" id="txtname" maxlength="40" placeholder="班级名称" class="" />
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group form-inline">
    ...
    <script type="text/javascript"> 
        ...
        */
        ...
        $('#gradelist').change(function () {
            ...
            $.ajax({
                type: "POST",  //请求方式
                url: "SchClassEdit.aspx/getgradeboss",  //请求路径：页面/方法名字
                data: params,     //参数
                dataType: "json",
...
<script type="text/javascript">
    ...
    {
        if($("#tecid_" + id).length > 0) return;
        var txtteclist = '';
        txtteclist += "<tr id=\""+id+"\">";
        txtteclist += "<td><input type=\"hidden\" id=\"tecid_"+id+"\" name=\"tecids\" value=\""+id+"\" /><div id=\"tecname_"+id+"\"  name=\"tecnames\">"+name+"</div></td>";
        txtteclist += "<td><select id=\"tecsub_"+id+"\" name=\"tecsubs\"><option value=\"\">请选择</option>"+subs+"</select></td>";
        txtteclist += "<td>";
        if(isms=="1")
        {
    ...
    function saveuser()
    {
        ...
        if(tecids.length>0)
        {
            for (var i = 0; i < tecids.length; i++) {
                ...
                sendstr+=$(tecids[i]).val()+','+$(tecnames[i]).html()+','+$(tecsubs[i]).val()+','+ims+'|';
            }
        ...
        $.ajax({
            type: "POST",  //请求方式
            url: "SchClassEdit.aspx/classsave",  //请求路径：页面/方法名字
            data: params,     //参数
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            success: function (data) {
